...
import { fetchMatches, joinMatch, fetchMyParticipantMatchIds } from '@/lib/supabaseClient';
...
export default function MatchesPage() {
  ...
  useEffect(() => {
    async function load() {
      try {
        ...
        const { data, error: matchesError } = await fetchMatches();

        if (matchesError) {
          console.error('Error loading matches:', matchesError);
          setError(matchesError.message ?? 'Unknown error');
        } else {
          setMatches((data ?? []) as Match[]);
        }

        if (session?.user) {
          const { data: joinedIds, error: joinedError } = await fetchMyParticipantMatchIds();
  ...
  async function handleJoin(matchId: string) {
    // ВАЖНО: редирект сразу, без “фейкового logout”
    ...
    try {
      const { data, error: joinError } = await joinMatch(matchId);

      if (joinError) {
        console.error('Error joining match:', joinError);
        setError(joinError.message ?? 'Failed to join match');
        return;
      }
      ...
      setMatches(prev => prev.map(m => (m.id === matchId ? (data as Match) : m)));
    } catch (err: any) {
    ...
    <>
      ...
      <main style={{ maxWidth: 768, margin: '0 auto', padding: 16 }}>
        <div style={{ padding: 20, color: 'white' }}>
          ...
          <div style={{ opacity: 0.7, fontSize: 14, marginBottom: 12 }}>Join a game in your area</div>

          {error && <p style={{ color: 'red', marginTop: 10 }}>Error: {error}</p>}

          {matches.length === 0 ? (
            <p style={{ opacity: 0.7 }}>No upcoming games yet.</p>
          ) : (
            <ul style={{ listStyle: 'none', padding: 0, marginTop: 10 }}>
              {matches.map(match => {
                ...
                const state: CardState = isFull ? 'full' : isJoined ? 'joined' : 'open';
                ...
                return (
                  <li
                    ...
                    style={{
                      ...
                      borderLeft: `4px solid ${
                        state === 'full' ? '#6b2a2a' : state === 'joined' ? '#2d5a2d' : '#2a2a2a'
                      }`,
                      background: '#151515',
                      borderRadius: 10,
                      width: '100%',
                  ...
                  >
                    <div style={{ minWidth: 0 }}>
                      ...
                      >
                        <div style={{ fontSize: 16, fontWeight: 700, lineHeight: 1.2 }}>{match.title}</div>
                      </Link>
                    ...
                    <button
                      ...
                      style={{
                        ...
                        borderRadius: 10,
                        border: `1px solid ${
                          state === 'full' ? '#6b2a2a' : state === 'joined' ? '#2d5a2d' : '#444'
                        }`,
                        background: state === 'full' ? '#3a1f1f' : state === 'joined' ? '#2d5a2d' : '#222',
                        color: 'white',
